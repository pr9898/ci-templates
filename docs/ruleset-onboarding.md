...
确保 `Yun-Hai-Org/ci-templates` 仓库的 `standard-ci.yml` 可正常调用（已完成）。
...
1. GitHub 自动在后台拉取并运行 `Yun-Hai-Org/ci-templates/.github/workflows/standard-ci.yml@v1`
...
3. `Yun-Hai-Org/ci-templates` 仓库的 `standard-ci.yml` 可被访问（公开仓库或同 org）
